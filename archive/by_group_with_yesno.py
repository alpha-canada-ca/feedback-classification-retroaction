def bygroup():
    ...
    if group == 'no_group':
        ...
        data['Lookup_group_EN'] = [
            ','.join(map(str, l)) for l in data['Lookup_group_EN']]
        ...
        if group_data.empty:
            ...
            group_data = group_data[["Comment", "Date", "Status",  "What's wrong", "Lookup_tags", 'Tags confirmed',
                                     'Yes/No', 'Lookup_page_title', 'URL_function', 'Lookup_FR_tag', "Lookup_group_EN", "Lookup_group_FR"]]
            ...
            group_data['Lookup_page_title'] = [
                ','.join(map(str, l)) for l in group_data['Lookup_page_title']]
            group_data['Lookup_group_FR'] = [
                ','.join(map(str, l)) for l in group_data['Lookup_group_FR']]
            ...
            yes_no_db['url'] = yes_no_db['url'].str.replace(
                'https://https://', 'https://')
            yes_no_db = yes_no_db[yes_no_db['url'].isin(
                list(urls['URL_function']))]
            ...
            if all_data.empty:
                ...
                if lang == 'en':
                    all_data['tags'] = [','.join(map(str, l))
                                        for l in all_data['Lookup_tags']]
                if lang == 'fr':
                    all_data['tags'] = [','.join(map(str, l))
                                        for l in all_data['Lookup_FR_tag']]
                ...
                for tag, topic_df in all_data.groupby(0):
                    tag_dico[tag] = topic_df[[
                        'Date', 'Comment', 'URL_function']]
                    ...
                    for tag, topic_dfn in all_data.groupby(1):
                        if tag_dico[tag].empty:
                            ...
                            tag_dico[tag] = tag_dico[tag].append(
                                topic_df[['Date', 'Comment', 'URL_function']])
                ...
                if confirmed.empty and unconfirmed.empty:
                    ...
                    if lang == "en":
                        unconfirmed['tags'] = [
                            ','.join(map(str, l)) for l in unconfirmed['Lookup_tags']]
                    if lang == "fr":
                        unconfirmed['tags'] = [
                            ','.join(map(str, l)) for l in unconfirmed['Lookup_FR_tag']]
                    ...
                    unconfirmed = unconfirmed.reset_index(drop=True)
                    unconfirmed_tags = unconfirmed["tags"].str.split(
                        ",", n=3, expand=True)
                    unconfirmed = unconfirmed.join(unconfirmed_tags)
                    unconfirmed = unconfirmed.drop(columns=['tags'])

                    unconfirmed_tag_count = unconfirmed_tags.apply(
                    ...
                    if 2 in unconfirmed_tag_count.columns:
                        unconfirmed_tag_count = unconfirmed_tag_count[0] + \
                            unconfirmed_tag_count[1] + unconfirmed_tag_count[2]
                    ...
                    for tag, topic_df in unconfirmed.groupby(0):
                        unconfirmed_tag_dico[tag] = topic_df[[
                            'Date', 'Comment', 'URL_function']]
                    ...
                    if lang == 'en':
                        confirmed['tags'] = [
                            ','.join(map(str, l)) for l in confirmed['Lookup_tags']]
                    if lang == 'fr':
                        confirmed['tags'] = [
                            ','.join(map(str, l)) for l in confirmed['Lookup_FR_tag']]
                    ...
                    over_tag_plots = {tag: tag_plots[tag]
                                      for tag in over_unique_tags}
                    over_plots = list(over_tag_plots.values())

                    under_tag_plots = {tag: tag_plots[tag]
                                       for tag in under_unique_tags}
