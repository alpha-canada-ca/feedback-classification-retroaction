def bypage():
    ...
    if lang == 'en':
        ...
        chart_columns = ['Date', 'Yes', 'No',
                         'Daily ratio', 'Weekly rolling mean']

    if lang == 'fr':
        tag_columns = ['Date', 'Commentaire']
        reason_column_names = ['Nombre de rétroactions',
                               'Raison', 'Mots significatifs']
        word_column_names = ['Nombre', 'Mots']
        chart_columns = ['Date', 'Oui', 'Non',
                         'Proportion quotidienne', 'Moyenne mobile sur 7 jours']
    ...
    if page == 'no_page':
        ...
        data['URL_function'] = data['URL_function'].str.replace(
            '/content/canadasite', 'www.canada.ca')
        data['URL_function'] = data['URL_function'].str.replace(
            'www.canada.ca', 'https://www.canada.ca')
        data['URL_function'] = data['URL_function'].str.replace(
            'https://https://', 'https://')
        ...
        if page_data.empty:
            ...
            if page_data['Lang'][0] == 'EN':
                ...
                page_data_en = page_data[["Comment", "Date", "Status",  "What's wrong", "Lookup_tags",
                                          'Tags confirmed', 'Yes/No', 'Lookup_page_title', 'Lookup_group_EN', 'Lookup_group_FR']]
                ...
                page_data_en['Lookup_page_title'] = [
                    ','.join(map(str, l)) for l in page_data_en['Lookup_page_title']]
                ...
                yes_no_db = yes_no_db[[
                    "url", "yesno", "problemDate", "problem"]]
                yes_no_db['url'] = yes_no_db['url'].str.replace(
                    '/content/canadasite', 'www.canada.ca')
                yes_no_db['url'] = yes_no_db['url'].str.replace(
                    'www.canada.ca', 'https://www.canada.ca')
                yes_no_db['url'] = yes_no_db['url'].str.replace(
                ...
                yes_no_db['problemDate'] = pd.to_datetime(
                    yes_no_db.problemDate.str.extract('^\w* ([\w]+ \d+ \d+)')[0])
                yes_no_db['problemDate'] = yes_no_db.problemDate.dt.strftime(
                    '%Y-%m-%d')
                yes_no_db = yes_no_db[yes_no_db['problemDate'] >= earliest]

                reasons = yes_no_db[['problem']]
                yes_no = yes_no_db[['problemDate', 'yesno']]
                yes_no = yes_no.rename(
                    columns={"problemDate": "Date", "yesno": "Yes/No"})
                ...
                for date in by_date:
                    by_date[date] = [by_date[date]['Yes'], by_date[date]['No'],
                                     (by_date[date]['Yes']/(by_date[date]['Yes'] + by_date[date]['No']))]

                df_yes = pd.DataFrame(list(by_date.values()), columns=[
                                      'Yes', 'No', 'Percentage'])
                df_yes['Date'] = list(by_date.keys())
                df_yes = df_yes[['Date', 'Yes', 'No', 'Percentage']]

                df_yes = df_yes.sort_values(by='Date')
                df_yes['Rolling mean'] = df_yes.iloc[:,
                                                     3].rolling(window=7).mean()
                ...
                if lang == 'en':
                    ...
                    plt.title(
                        'Ratio of people who said they found their answer')
                ...
                if lang == 'fr':
                    ...
                    plt.title(
                        'Proportion de gens qui disent avoir trouver leur réponse')
                ...
                yes_no_period = yes_no_db[yes_no_db['problemDate']
                                          >= start_date]

                yes_no_period = yes_no_period[['problemDate', 'yesno']]
                yes_no_period = yes_no_period.rename(
                    columns={"problemDate": "Date", "yesno": "Yes/No"})
                yes_no_period = yes_no_period.dropna()
                yes_no_period = yes_no_period.sort_values(
                ...
                if yes_no_period.empty:
                    ...
                    if score_period > score:
                        delta = '+' + \
                            format(float(score_period)-float(score), '.2f')
                ...
                if all_data_en.empty:
                    ...
                    by_tag = by_tag.sort_index(
                        axis=0, level=None, ascending=True)
                    by_tag.columns = ['Feedback count']

                    by_tag = by_tag.sort_values(
                        by='Feedback count', ascending=False)
                    ...
                    if 1 in page_data_en.columns:
                        for tag, topic_df_en in all_data_en.groupby(1):
                            if tag_dico[tag].empty:
                                tag_dico[tag] = topic_df_en[[
                                    'Date', 'Comment']]
                            else:
                                tag_dico[tag] = tag_dico[tag].append(
                                    topic_df_en[['Date', 'Comment']])
                    ...
                    for tag in tag_dico:
                        tag_dico[tag] = tag_dico[tag].sort_values(
                            by='Date', ascending=False)
                    ...
                    for tag in tag_dates:
                        idx = pd.date_range(
                            date_range.iloc[0], date_range.iloc[-1])
                        tag_dates[tag].index = pd.DatetimeIndex(
                            tag_dates[tag].index)
                        tag_dates[tag] = tag_dates[tag].reindex(
                            idx, fill_value=0)
                    ...
                    for tag in tag_dates:
                        ...
                        tag_dates[tag]['Rolling mean'] = tag_dates[tag].iloc[:, 1].rolling(
                            window=7).mean()
                        ...
                        fig, ax = plt.subplots()
                        if lang == 'en':
                            ax.bar(x, y1, color=(0.2, 0.4, 0.6, 0.6),
                                   linewidth=0.5, label='Daily value')
                            ax.plot(x, y2, color='black', linewidth=3.0,
                        ...
                        if lang == 'fr':
                            ax.bar(x, y1, color=(0.2, 0.4, 0.6, 0.6),
                                   linewidth=0.5, label='Valeur quotidienne')
                            ax.plot(x, y2, color='black', linewidth=3.0,
                                    label='Moyenne mobile sur 7 jours')
                            plt.title(
                                tag + '\n' + 'Nombre de commentaires par jour   ')

                        plt.axvspan(start_plot, end_plot,
                        ...
                        plt.close()
                        img.seek(0)
                        tag_plots[tag] = base64.b64encode(
                            img.getvalue()).decode()
                    ...
                    if page_data_en.empty:
                        ...
                        page_data_en = page_data_en.drop(
                            columns=['Tags confirmed'])
                        ...
                        page_data_en['tags'] = [
                            ','.join(map(str, l)) for l in page_data_en['Lookup_tags']]
                        tags_en = page_data_en["tags"].str.split(
                            ",", n=3, expand=True)
                        ...
                        all_words_en = ' '.join(
                            [str(elem) for elem in word_list_en])
                        ...
                        page_data_en[["What's wrong"]] = page_data_en[[
                            "What's wrong"]].replace([False], ['None'])
                        page_data_en[["What's wrong"]] = page_data_en[["What's wrong"]].replace(
                        ...
                        for reason, topic_df_en in page_data_en.groupby("What's wrong"):
                            reason_dict[reason] = ' '.join(
                                topic_df_en['Comment'].tolist())

                        tokenizer = nltk.RegexpTokenizer(r"\w+")

                        for value in reason_dict:
                            reason_dict[value] = tokenizer.tokenize(
                                reason_dict[value])
                        ...
                        reason_words_en = [[word.lower() for word in value]
                                           for value in reason_words_en]
                        reason_words_en = [[lemmatizer.lemmatize(
                            word) for word in value] for value in reason_words_en]
                        reason_words_en = [
                            [word for word in value if word not in sw] for value in reason_words_en]
                        reason_words_en = [
                            [word for word in value if word.isalpha()] for value in reason_words_en]

                        from gensim.corpora.dictionary import Dictionary

                        reason_dictionary_en = Dictionary(reason_words_en)

                        reason_corpus_en = [reason_dictionary_en.doc2bow(
                            reason) for reason in reason_words_en]

                        from gensim.models.tfidfmodel import TfidfModel

                        reason_tfidf_en = TfidfModel(reason_corpus_en)

                        reason_tfidf_weights_en = [sorted(
                            reason_tfidf_en[doc], key=lambda w: w[1], reverse=True) for doc in reason_corpus_en]

                        reason_weighted_words_en = [[(reason_dictionary_en.get(
                            id), weight) for id, weight in ar] for ar in reason_tfidf_weights_en]

                        reason_imp_words_en = pd.DataFrame(
                            {'Reason': reason_list_en, 'EN_words':  reason_weighted_words_en})

                        reason_imp_words_en = reason_imp_words_en.sort_values(
                            by='Reason')

                        reason_imp_words_en = reason_imp_words_en.reset_index(
                            drop=True)

                        reason_imp_words_en['EN_words'] = reason_imp_words_en['EN_words'].apply(
                            lambda x: list(x))
                        ...
                        by_reason['Significant words'] = by_reason['Significant words'].apply(
                            lambda x: ', '.join(x))

                        by_reason = by_reason[[
                            'Feedback count', 'index', 'Significant words']]
                        ...
                        for tag in unique_tags:
                            tag_dico[tag] = pd.DataFrame(
                                columns=tag_dico_columns)
                        ...
                        over_dict = {key: tag_dico[key]
                                     for key in over_unique_tags}
                        under_dict = {key: tag_dico[key]
                                      for key in under_unique_tags}

                        tag_columns = ['Date', 'Comment']

                        over_tag_plots = {
                            tag: tag_plots[tag] for tag in over_unique_tags}
                        over_plots = list(over_tag_plots.values())

                        under_tag_plots = {
                            tag: tag_plots[tag] for tag in under_unique_tags}
                        ...
                        if unconfirmed_en.empty:
                            ...
                            unconfirmed_en = unconfirmed_en.reset_index(
                                drop=True)
                            unconfirmed_en['tags'] = [
                                ','.join(map(str, l)) for l in unconfirmed_en['Lookup_tags']]
                            unconfirmed_en = unconfirmed_en.drop(
                                columns=['Lookup_tags'])
                            unconfirmed_en = unconfirmed_en.drop(
                                columns=['Tags confirmed'])
                            unconfirmed_en = unconfirmed_en.reset_index(
                                drop=True)
                            unconfirmed_tags_en = unconfirmed_en["tags"].str.split(
                                ",", n=3, expand=True)
                            unconfirmed_en = unconfirmed_en.join(
                                unconfirmed_tags_en)
                            unconfirmed_en = unconfirmed_en.drop(
                                columns=['tags'])

                            unconfirmed_tag_count = unconfirmed_tags_en.apply(
                                pd.Series.value_counts)
                            unconfirmed_tag_count = unconfirmed_tag_count.fillna(
                                0)
                            unconfirmed_tag_count = unconfirmed_tag_count.astype(
                                int)
                            if 2 in unconfirmed_tag_count.columns:
                                unconfirmed_tag_count = unconfirmed_tag_count[0] + \
                                    unconfirmed_tag_count[1] + \
                                    unconfirmed_tag_count[2]
                            elif 1 in unconfirmed_tag_count.columns:
                                unconfirmed_tag_count = unconfirmed_tag_count[0] + \
                                    unconfirmed_tag_count[1]
                            else:
                                unconfirmed_tag_count = unconfirmed_tag_count[0]
                            unconfirmed_tag_count = unconfirmed_tag_count.sort_values(
                                ascending=False)
                            unconfirmed_by_tag = unconfirmed_tag_count.to_frame()
                            unconfirmed_by_tag = unconfirmed_by_tag.sort_index(
                                axis=0, level=None, ascending=True)
                            unconfirmed_by_tag.columns = ['Feedback count']

                            unconfirmed_by_tag = unconfirmed_by_tag.sort_values(
                                by='Feedback count', ascending=False)
                            unconfirmed_unique_tags = list(
                                unconfirmed_by_tag.index)
                            unconfirmed_tag_dico = {}

                            tag_dico_columns = ['Date', 'Comment']

                            for tag in unconfirmed_unique_tags:
                                unconfirmed_tag_dico[tag] = pd.DataFrame(
                                    columns=tag_dico_columns)

                            for tag, topic_df_en in unconfirmed_en.groupby(0):
                                unconfirmed_tag_dico[tag] = topic_df_en[[
                                    'Date', 'Comment']]
                            ...
                            if 1 in unconfirmed_en.columns:
                                for tag, topic_df_en in unconfirmed_en.groupby(1):
                                    if unconfirmed_tag_dico[tag].empty:
                                        ...
                                        unconfirmed_tag_dico[tag] = unconfirmed_tag_dico[tag].append(
                                            topic_df_en[['Date', 'Comment']])
                            ...
                            for tag in unconfirmed_tag_dico:
                                unconfirmed_tag_dico[tag] = unconfirmed_tag_dico[tag].sort_values(
                                    by='Date', ascending=False)

                            unconfirmed_dict = {
                                key: unconfirmed_tag_dico[key] for key in unconfirmed_unique_tags}

                            tag_columns = ['Date', 'Comment']

                            unconfirmed_tag_plots = {
                                tag: tag_plots[tag] for tag in unconfirmed_unique_tags}
                            unconfirmed_plots = list(
                                unconfirmed_tag_plots.values())
            ...
            # process to follow if French
                ...
                page_data_fr = page_data[["Comment", "Date", "Status",  "What's wrong", "Lookup_FR_tag",
                                          'Tags confirmed', 'Yes/No', 'Lookup_page_title', 'Lookup_group_EN', 'Lookup_group_FR']]
                ...
                page_data_fr['Lookup_page_title'] = [
                    ','.join(map(str, l)) for l in page_data_fr['Lookup_page_title']]
                page_data_fr['Lookup_group_EN'] = [
                    ','.join(map(str, l)) for l in page_data_fr['Lookup_group_EN']]
                page_data_fr['Lookup_group_FR'] = [
                    ','.join(map(str, l)) for l in page_data_fr['Lookup_group_FR']]
                ...
                for date in by_date:
                    by_date[date] = [by_date[date]['Yes'], by_date[date]['No'],
                                     (by_date[date]['Yes']/(by_date[date]['Yes'] + by_date[date]['No'])) * 100]
                ...
                if lang == 'en':
                    ...
                    ax.plot(x, y2, linewidth=3.0, label='Weekly rolling mean')
                    plt.title(
                        'Percentage of people who said they found their answer')
                ...
                if lang == 'fr':
                    ...
                    ax.plot(x, y2, linewidth=3.0,
                            label='Moyenne mobile sur 7 jours')
                    plt.title(
                        'Pourcentage de gens qui disent avoir trouver leur réponse')
                plt.axvspan(start_plot, end_plot, color='blue', alpha=0.3)
                plt.legend()
                loc = plticker.MultipleLocator(base=7.0)
                plt.gcf().subplots_adjust(bottom=0.2)
                ...
                if yes_no_period.empty:
                    ...
                    score_period = (
                        yes_period / (yes_period + no_period)) * 100
                    score_period = format(score_period, '.2f')
                    if score_period > score:
                        delta = '+' + \
                            format(float(score_period)-float(score), '.2f')
                ...
                if all_data_fr.empty:
                    ...
                    all_data_fr['tags'] = [
                        ','.join(map(str, l)) for l in all_data_fr['Lookup_FR_tag']]

                    # remove the Lookup_tags column (it's not needed anymore)
                    all_data_fr = all_data_fr.drop(columns=['Lookup_FR_tag'])
                    all_tags_fr = all_data_fr["tags"].str.split(
                        ",", n=3, expand=True)
                    ...
                    if 1 in page_data_fr.columns:
                        for tag, topic_df_fr in all_data_fr.groupby(1):
                            if tag_dico[tag].empty:
                                tag_dico[tag] = topic_df_fr[[
                                    'Date', 'Comment']]
                            else:
                                tag_dico[tag] = tag_dico[tag].append(
                                    topic_df_fr[['Date', 'Comment']])
                    ...
                    if page_data_fr.empty:
                        ...
                        # converts the tags to a string (instead of a list) - needed for further processing - and puts it in a new column
                        page_data_fr['tags'] = [
                            ','.join(map(str, l)) for l in page_data_fr['Lookup_FR_tag']]

                        # remove the Lookup_FR_tag column (it's not needed anymore)
                        page_data_fr = page_data_fr.drop(
                            columns=['Lookup_FR_tag'])
                        ...
                        # split tags and expand
                        tags_fr = page_data_fr["tags"].str.split(
                            ",", n=3, expand=True)
                        ...
                        all_words_fr = ' '.join(
                            [str(elem) for elem in word_list_fr])
                        ...
                        most_common = fdist1.most_common(15)
                        mc = pd.DataFrame(most_common, columns=[
                                          'Mots', 'Nombre'])
                        mc = mc[['Nombre', 'Mots']]
                        ...
                        # by what's wrong reason
                        page_data_fr[["What's wrong"]] = page_data_fr[[
                            "What's wrong"]].replace([False], ['Aucun'])
                        page_data_fr[["What's wrong"]] = page_data_fr[["What's wrong"]].replace(
                            ["The information isn't clear"], ["The information isn’t clear"])
                        page_data_fr[["What's wrong"]] = page_data_fr[["What's wrong"]].replace(
                            ["I'm not in the right place"], ["I’m not in the right place"])
                        page_data_fr[["What's wrong"]] = page_data_fr[["What's wrong"]].replace(
                        ...
                        reason_words_fr = [[word.lower() for word in value]
                                           for value in reason_words_fr]
                        reason_words_fr = [[lemmatizer.lemmatize(
                            word) for word in value] for value in reason_words_fr]
                        reason_words_fr = [
                            [word for word in value if word not in sw] for value in reason_words_fr]
                        reason_words_fr = [
                            [word for word in value if word.isalpha()] for value in reason_words_fr]

                        from gensim.corpora.dictionary import Dictionary

                        reason_dictionary_fr = Dictionary(reason_words_fr)

                        reason_corpus_fr = [reason_dictionary_fr.doc2bow(
                            reason) for reason in reason_words_fr]

                        from gensim.models.tfidfmodel import TfidfModel

                        reason_tfidf_fr = TfidfModel(reason_corpus_fr)

                        reason_tfidf_weights_fr = [sorted(
                            reason_tfidf_fr[doc], key=lambda w: w[1], reverse=True) for doc in reason_corpus_fr]

                        reason_weighted_words_fr = [[(reason_dictionary_fr.get(
                            id), weight) for id, weight in ar] for ar in reason_tfidf_weights_fr]

                        reason_imp_words_fr = pd.DataFrame(
                            {'Reason': reason_list_fr, 'FR_words':  reason_weighted_words_fr})

                        reason_imp_words_fr = reason_imp_words_fr.sort_values(
                            by='Reason')

                        reason_imp_words_fr = reason_imp_words_fr.reset_index(
                            drop=True)

                        reason_imp_words_fr['FR_words'] = reason_imp_words_fr['FR_words'].apply(
                            lambda x: list(x))
                        ...
                        under_tag_plots = {
                            tag: tag_plots[tag] for tag in under_unique_tags}
                        under_plots = list(under_tag_plots.values())
                        column_names = ['Nombre de rétroactions',
                                        'Étiquette', 'Mots significatifs']
                        ...
                        if unconfirmed_fr.empty:
                            ...
                            unconfirmed_fr = unconfirmed_fr.reset_index(
                                drop=True)
                            unconfirmed_fr['tags'] = [
                                ','.join(map(str, l)) for l in unconfirmed_fr['Lookup_FR_tag']]
                            unconfirmed_fr = unconfirmed_fr.drop(
                                columns=['Lookup_FR_tag'])
                            unconfirmed_fr = unconfirmed_fr.drop(
                                columns=['Tags confirmed'])
                            unconfirmed_fr = unconfirmed_fr.reset_index(
                                drop=True)
                            unconfirmed_tags_fr = unconfirmed_fr["tags"].str.split(
                                ",", n=3, expand=True)
                            unconfirmed_fr = unconfirmed_fr.join(
                                unconfirmed_tags_fr)
                            unconfirmed_fr = unconfirmed_fr.drop(
                                columns=['tags'])

                            unconfirmed_tag_count = unconfirmed_tags_fr.apply(
                                pd.Series.value_counts)
                            unconfirmed_tag_count = unconfirmed_tag_count.fillna(
                                0)
                            unconfirmed_tag_count = unconfirmed_tag_count.astype(
                            ...
                            for tag, topic_df_fr in unconfirmed_fr.groupby(0):
                                unconfirmed_tag_dico[tag] = topic_df_fr[[
                                    'Date', 'Comment']]
